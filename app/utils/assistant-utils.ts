export async function checkAssistantPrerequisites() {
  ...
  console.log(
    `Checking prerequisites: API Key: ${
      PINECONE_API_KEY ? "Set" : "Not Set"
    }, Assistant Name: ${PINECONE_ASSISTANT_NAME}`
  );

  if (!PINECONE_API_KEY || !PINECONE_ASSISTANT_NAME) {
    console.error(
      "Error: PINECONE_API_KEY or PINECONE_ASSISTANT_NAME is missing."
    );
